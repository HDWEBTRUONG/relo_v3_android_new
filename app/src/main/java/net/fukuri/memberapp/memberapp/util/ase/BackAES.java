package net.fukuri.memberapp.memberapp.util.ase;
...
public class BackAES
{
	...
	public static byte[] encrypt(String sSrc, String sKey, int type) throws Exception
	{
		...
		IvParameterSpec iv = new IvParameterSpec(ivParameter.getBytes());// Using the CBC mode requires a vector iv to increase the strength of the encryption algorithm
		if (isPwd == false)
		{// ECB does not have a password
		...
		return Base64.encode(encrypted);// BASE64 is used here to do transcoding.
	}
	...
	public static String decrypt(String sSrc, String sKey, int type) throws Exception
	{
		...
		try
		{
			...
			byte[] encrypted1 = Base64.decode(sSrc.getBytes());// First decrypt with base64
	...
	public static byte[] newencrypt(String content, String password)
	{
		try
		{
			...
			Cipher cipher = Cipher.getInstance("AES");// Create an AES Encryption Encoder
			byte[] byteContent = content.getBytes("UTF-8");
			cipher.init(Cipher.ENCRYPT_MODE, key);// Initialize AES encryption
			byte[] result = cipher.doFinal(byteContent);
			return result; // AES encryption results
		} catch (Exception e)
		{
			e.printStackTrace();
		}
	...
	public static byte[] newdecrypt(byte[] content, String password)
	{
		try
		{
			...
			SecretKeySpec key = new SecretKeySpec(enCodeFormat, "AES");
			Cipher cipher = Cipher.getInstance("AES");// Create an AES Encryption Encoder
			cipher.init(Cipher.DECRYPT_MODE, key);// Initialize AES encryption
			byte[] result = cipher.doFinal(content);
			return result; // Get the result of AES decryption
		} catch (Exception e)
		{
			e.printStackTrace();
		}
